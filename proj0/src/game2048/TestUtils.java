...
public class TestUtils {
    ...
    public static void updateModel(int[][] values, int score, int maxScore, boolean gameOver) {
        ...
        assert values[0].length == TestUtils.SIZE : "board must have 4x4 dimensions";
        TestUtils.model = new Model(values, score, maxScore, gameOver);
    }
    ...
    public static void checkChanged(Side s, boolean expected, boolean actual) {
        ...
        if (expected) {
            changedErrMsg = TestUtils.boardShouldChange(s);
            assertTrue(changedErrMsg, actual);
        } else {
            changedErrMsg = TestUtils.boardShouldNotChange(s);
