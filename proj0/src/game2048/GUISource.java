...
class GUISource implements InputSource {
    ...
    /**
     * Provides input from SOURCE, logging it iff LOG. Use RANDOMSOURCE to select random tiles
     * having value 2 with probability PROBOF2.
     */
    GUISource(GUI source, Random randomSource, double probOf2, String logFileName) {
        this._source = source;
        this._randomSource = randomSource;
        this._probOf2 = probOf2;

        if (logFileName != null) {

            File logFile = new File(logFileName);
            try {
                this._logFileWriter = new FileWriter(logFile);
            } catch (IOException e) {
    ...
    public String getKey() {
        String command = this._source.readKey();
        switch (command) {
            case "↑":
                command = "Up";
                break;
        ...
        if (this._logFileWriter != null) {
            ...
            try {
                this._logFileWriter.write(logLine);
            } catch (IOException e) {
    ...
    @Override
    /**
     * Return a randomly positioned tile with either value of 2 with probability _probOf2 or a value
     * of 4 with probability 1 - _probOf2 in a board with size SIZE.
     */
    public Tile getNewTile(int size) {
        int c = this._randomSource.nextInt(size), r = this._randomSource.nextInt(size);
        int v = this._randomSource.nextDouble() <= this._probOf2 ? 2 : 4;
        if (this._logFileWriter != null) {
