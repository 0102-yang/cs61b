...
public class GuitarPlayer {
    ...
    public GuitarPlayer(InputStream source) {
        try {
            this.sequence = MidiSystem.getSequence(source);
    ...
    private void initialize() {
        this.strings = new GuitarString[128];
        this.vol = new double[128];
        for (int i = 0; i < this.strings.length; i++) {
            this.strings[i] = new GuitarString(440.0 * Math.pow(2.0, (i - 69.0) / 12.0));
            this.vol[i] = 0.0;
        }
    }
    ...
    private double sample() {
        double sum = 0.0f;
        for (int i = 0; i < this.strings.length; i++) {
            sum += this.vol[i] * this.strings[i].sample();
        }
        return sum;
    }
    ...
    public void play() {
        ...
        initialize();
        double bpm = 120;
        double samplesPerTick = StdAudio.SAMPLE_RATE * (60.0 / (this.sequence.getResolution() * bpm));
        ...
        for (int i = 0; i < track.size(); i++) {
            ...
            if (msg instanceof MetaMessage) {
                ...
                if (mm.getType() == 0x51) {
                    ...
                    samplesPerTick = StdAudio.SAMPLE_RATE
                            * (60.0 / (this.sequence.getResolution() * bpm));
            ...
            while (j < data.length - 2) {
                ...
                if (s >= 0x80 && s <= 0x8F) {
                    ...
                    int note = data[j++] & 0xFF;
                    int vel = data[j++] & 0xFF;
                    this.vol[note] = vel / 127.0;
                    this.strings[note].pluck();
                } else {
                    // status
                    int d = data[j++] & 0xFF;
                    int d2 = data[j++] & 0xFF;
